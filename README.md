...
*Zerberus Pro 4.0 – Stand: 2026-04-28, Patch 175 (**Phase E abgeschlossen**: NalaAdapter + Policy-Engine + RosaAdapter-Stub + Trust-Boundary-Diagramm. `core/policy_engine.py` mit abstraktem Interface + `HuginnPolicy`-Fassade die Rate-Limiter (P163), Sanitizer (P162/P173) und HitL-Policy (P164) zu einer einzigen `PolicyDecision` aggregiert (`ALLOW`/`DENY`/`ESCALATE` × Severity-Mapping per Trust-Level). `adapters/nala_adapter.py` mit Permission-Level→Trust-Mapping aus dem JWT-Payload. `adapters/rosa_adapter.py` als Phase-F-Anker. `docs/trust_boundary_diagram.md` als Übersicht der Schichten. 37 neue Tests, alle bestehenden grün — Skelett für Rosa/Heimdall steht.)*

...
*Zerberus Pro 4.0 – Stand: 2026-04-28, Patch 176 (**Stabilisierungs-Patch zwischen Phase E und F**: Coda-Autonomie-Regel als Doku-Anker (`docker pull`, `pip install`, Sync — Coda macht alles was er kann), Sandbox erstmals operationsfähig (`python:3.12-slim`+`node:20-slim` gepullt, Healthcheck grün), Test-Isolation aufgeräumt: 5 Playwright-Files mit `@pytest.mark.e2e`, neue `pytest.ini` mit `addopts = -m "not e2e and not guard_live"` plus `filterwarnings`, `test_dialect_ui` Slice-Window 6000→8000. **Default-Run: 954 passed, 114 deselected, 0 failed in 29s.** `pytest -m e2e -v` für Loki/Fenrir/Vidar mit laufendem Server, `pytest -m guard_live` für Mistral-Live-Tests mit `OPENROUTER_API_KEY`.)*

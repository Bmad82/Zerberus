...
**P203b — Hel-UI-Hotfix.** Chris-Bugmeldung 2026-05-02 als BLOCKER vor allen anderen Patches: Hel rendert, aber NICHTS ist anklickbar (Tabs wechseln nicht, Buttons reagieren nicht, Forms nicht bedienbar, Settings-Zahnrad öffnet kein Panel). Nala unauffällig. Aufgefallen erst nach P200/P202 — aber Bug existiert seit P196. Browser-/SW-Cache hatte vorher eine ältere Hel-Version aus der Zeit vor P196 ausgeliefert; Chris' manuelles Unregister + Cache-Wipe + Hard-Refresh nach P202 hat dann das echte aktuelle ADMIN_HTML mit dem P196-Bug freigelegt.

**Root-Cause-Diagnose:**

Die zentrale `<script>`-Funktion `loadProjectFiles` (eingeführt mit P196) enthielt:
...
- `,\''` → Python interpretiert `\'` als `'` (escape) und das nächste `'` als String-Close → ergibt `,''` (3 chars: comma, quote, quote)
- `"\\'"` → Python interpretiert `\\` als `\` und `\'` als `'` → ergibt `"\'"` (in JS unsinnig: `"\'"` ist ein 1-char string `'`)
- `'\')" '` → `')" `

Der ausgelieferte JS-String wurde damit zu:

```js
+ '<button onclick="deleteProjectFile(' + projectId + ',' + f.id + ','' + _escapeHtml(f.relative_path).replace(/'/g, "\'") + '')" '
```

JavaScript parst `+ ',' '' +` als zwei adjacent String-Literale ohne Operator dazwischen → **harter SyntaxError ("Unexpected string")**. Ein einziger Syntax-Fehler in einem `<script>`-Block invalidiert den **GESAMTEN** Block — alle Funktionsdeklarationen darin werden nicht registriert. Das betraf den zentralen Block in [`hel.py`](zerberus/app/routers/hel.py) (Z. 1249-3414) mit hunderten Funktionen inkl. `activateTab`, `toggleHelSettings`, `loadMetrics`, `loadProfiles`, `renderModelSelect`. Klicks auf Tabs riefen `onclick="activateTab('llm')"` auf — die Funktion existierte aber nie → ReferenceError → Klick ignoriert. Genau Chris' Symptom.

**Fix-Architektur:**

Inline `onclick="fn(...)"` mit String-Concat über benutzergenerierte Daten ist eine fragile Konstruktion — XSS-anfällig (Filename mit `"`), schwer zu eskapen, abhängig vom Render-Kontext. Statt das Quote-Escaping zu reparieren (was die Pattern-Fragilität nicht beseitigt), wurde der Renderer auf **Event-Delegation mit `data-*`-Attributen** umgestellt:

```js
// Nachher (P203b):
'<button class="proj-file-delete-btn"'
...
Quote-immun (Filename geht durch `_escapeHtml` direkt ins Attribut), XSS-sicher (Attribut-Quoting + `_escapeHtml` für `&<>"'`), passt zum bestehenden `_setupProjectFileDrop`-Pattern (das nutzt schon `dataset.projectId`).

**Tests P203b (10 neu):**

- `TestBrokenQuotePatternAbsent` (3): keine `onclick="deleteProjectFile(`, kein `+ ',''` (3 adjacent quotes), kein `replace(/'/g, "\\'")` mehr im rendered HTML.
...
- `TestJsSyntaxIntegrity` (1): extrahiert ALLE inline `<script>`-Blöcke aus `ADMIN_HTML` und ruft `node --check` auf jedem auf. Skipped wenn `node` nicht im PATH. **Hätte den P196-Bug sofort gefangen** — als Insurance gegen Wiederholung eingebaut.
...
- **Kein globaler Refactor von `onclick="..."` auf Event-Delegation.** ADMIN_HTML hat noch dutzende inline `onclick`-Attribute — die meisten ohne String-Concat (`onclick="loadModels()"` o.ä.) und damit nicht fragil. Refactoring nur des Bug-Pfads.
- **Kein `node --check`-Pass über NALA_HTML.** Wäre sinnvoll als Schwester-Test, ist aber nicht von der Bug-Meldung abgedeckt. Kann in einen späteren Hardening-Patch.
- **Keine Behebung der doppelten `_escapeHtml`-Definitionen** (Z. 1653 + Z. 3096). Beide existieren seit länger, JS überschreibt im non-strict Mode silent. Erklärt nicht den Bug, ist aber ein Sauberkeits-Schmerzpunkt für später.

## Nächster Schritt — sofort starten

**P203c: Sandbox-Workspace-Mount + Code-Execution-Pipeline.** Eigentlich war P203b für Sandbox-Mount geplant (siehe alter HANDOVER unter P203a). Wegen des Hel-UI-Blockers wurde die Patch-Nummer für den Hotfix vergeben — der Sandbox-Mount-Patch heißt jetzt **P203c**. Inhaltlich unverändert:
...
4. **HitL-Gate kommt erst P205.** P203c läuft direkt durch — sicher, weil Workspace-Mount Read-Only-default und Sandbox bereits hart-isoliert ist.
...
- Manuelle Tests in WORKFLOW.md offen: #2-4 (Prosodie/Triptychon, pre-P194), #5-7 (P194), #8-10 (P195), #11-16 (P196), #17-18 (P197), #19-21 (P198), #22-25 (P199), #26-32 (P200), #33-37 (P201), #38-41 (P202), #42-45 (P203a), **#46-48 (P203b: Push, Hel-UI wieder bedienbar live, Datei-Lösch-Button mit Apostroph-Filename)**.
- Schulden in WORKFLOW.md unten: interactions-User-Spalte (Alembic), 2 pre-existing Test-Failures (`edge-tts` + `test_rag_dual_switch.test_fallback_logic`) — **lokaler Stand 1635 baseline + 10 neu = 1645 passed, 2 failed (beide pre-existing aus Schuldenliste)**, 4 xfailed sichtbar, 1 skipped — nicht blockierend.
...
- **`onclick`-Attribute mit String-Concat in HTML-Strings** — Pattern war fragil, P203b hat NUR den `loadProjectFiles`-Renderer gefixt. Andere Renderer in `ADMIN_HTML` mit ähnlichem Pattern sollten beim nächsten Touch auf Event-Delegation migriert werden. JS-Integrity-Test fängt Symptom (Syntax-Fehler), aber nicht das Pattern selbst.

...
## Input-Sanitizer + Telegram-Hardening (Patch 162)
- **`structlog` ist NICHT installiert.** Patch-Prompts aus dem 7-LLM-Review schlagen oft `structlog.get_logger()` vor. Zerberus nutzt durchgehend `logging.getLogger(...)`. Nicht blind übernehmen — auf das vorhandene Logger-Setup adaptieren (gleicher Stil wie [`bot.py`](zerberus/modules/telegram/bot.py)/[`hitl.py`](zerberus/modules/telegram/hitl.py)).
- **Sanitizer-Findings werden geloggt, nicht geblockt.** Im Huginn-Modus (Patch 162) lässt der `RegexSanitizer` Injection-Pattern-Treffer durch — der Guard (Mistral Small) entscheidet final. Grund: False-Positive-Risiko bei Regex auf normales Deutsch ist real, und Huginn lebt von der Persona/Sarkasmus, was Regex sowieso oft nicht von einem Jailbreak unterscheiden kann. Der `blocked=True`-Pfad ist im Code vorhanden, aber für Rosa reserviert (config-driven).
- **Singleton mit Test-Reset.** `get_sanitizer()` ist ein Modul-Singleton wie `get_settings()`. Tests müssen den State zwischen Runs resetten (`_reset_sanitizer_for_tests()` als autouse-Fixture in [`test_input_sanitizer.py`](zerberus/tests/test_input_sanitizer.py)) — sonst leakt eine in einem Test instantiierte Implementierung in den nächsten.
- **Persistente Offset-Datei kontaminiert ältere Tests.** `data/huginn_offset.json` wird vom `long_polling_loop` jetzt geschrieben. Ohne `monkeypatch.setattr(bot_module, "OFFSET_FILE", tmp_path / "off.json")` schreibt jeder Test, der den Loop startet, in die echte Datei und beeinflusst `_load_offset()` in späteren Tests. Pre-existierende Polling-Tests (`test_long_polling_loop_advances_offset`, `test_long_polling_handler_exception_does_not_break_loop`) brauchten die `tmp_path`-Patch nachträglich. Lesson: bei jeder neuen Modul-Level-Konstante mit Datei-IO sofort an Test-Isolation denken.
- **Telegram-Update-Typ-Filter gehört GANZ oben in `process_update()`.** Vor dem Event-Bus, vor dem Manager-Setup, vor allem. Sonst verbraucht die Pipeline für triviale Filter-Cases (channel_post, edited_message, poll) unnötig Setup-Zeit. Reihenfolge in [`router.py`](zerberus/modules/telegram/router.py): `enabled`-Check → Update-Typ-Filter → Bus → Manager → Callback → extract_message_info.
- **Edited-Messages sind ein Jailbreak-Vektor.** Wer eine harmlose Nachricht editiert auf „ignore previous instructions" und Webhook erneut verarbeitet, kriegt eine zweite Antwort die der Sanitizer auch noch kommentiert. Telegram liefert `edited_message`-Updates per Webhook auch wenn nicht explizit angefordert — der Filter im `process_update()` ist deshalb Pflicht, nicht „nice to have".
- **Callback-Spoofing in Gruppen.** Telegram validiert nicht, wer einen Inline-Button klickt — das muss der Bot tun. Vor Patch 162 war HitL nur Admin-DM, also implizit sicher. Mit `requester_user_id` im `HitlRequest` ist die Validierung jetzt für zukünftige In-Group-Buttons vorbereitet (z. B. „User klickt seine eigene Code-Run-Bestätigung"). Erlaubte Klicker: `{admin_chat_id, requester_user_id}` — beide String-vergleichen, weil Telegram `from.id` als int liefert aber Config-Felder oft strings sind.
- **`message_thread_id` muss durchgereicht werden, nicht nur extrahiert.** Forum-Gruppen (Topics) verlieren sonst den Kontext: Antwort landet im General statt im richtigen Thread. `extract_message_info()` exposed jetzt das Feld, ALLE `send_telegram_message`-Calls in `router.py` reichen es als kwarg durch. Pattern: `message_thread_id=info.get("message_thread_id")` immer mitgeben, Telegram ignoriert den Key wenn None — wir setzen ihn deshalb nur ins Payload, wenn er truthy ist.

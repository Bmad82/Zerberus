...
- Custom-pytest-Marker brauchen Registrierung (P171): `@pytest.mark.docker` ohne `pytest_configure` → `PytestUnknownMarkWarning` und Marker-Filter (`pytest -m docker`) funktioniert weniger sauber|Registrierung in `conftest.py::pytest_configure` mit `config.addinivalue_line("markers", "...")`|kombiniert mit `skipif` für Auto-Skip wenn Voraussetzung fehlt (Docker-Daemon)

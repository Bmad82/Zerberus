...
- **Uvicorn `--reload` ist auf Windows bei dieser Projektkonfiguration generell unzuverlässig** — in einer einzigen Session (Patches 95, 97, 98) jedes Mal manueller Kill + Neustart nötig, weil Änderungen an `nala.py`, `orchestrator.py`, `router.py` nicht ankamen. Symptome: neue Marker im HTML fehlen, neue Log-Tags fehlen (`[EXPAND-97]` nicht sichtbar trotz aktivierter Config). `touch <file>` hilft nicht. Watchfiles läuft zwar, aber der Reload-Trigger zündet nicht im erwarteten Rhythmus. **Pragmatisches Verfahren für Eval/Smoke-Tests:** nach jedem RAG-/Frontend-Patch `taskkill //PID <uvicorn-worker> //F` + neuer `uvicorn … --reload &`-Start (braucht ~15 s bis Ready). Alternative wäre `--reload-dir zerberus` oder `watchfiles` als separater Watcher — noch nicht evaluiert (Patches 89/95/97/98)
- **Zombie-Uvicorn-Worker durch wiederholte `taskkill`+`--reload`-Zyklen (Patch 99):** Mehrere parallele uvicorn-Master-Trees können entstehen, wenn ältere Kill-Versuche nicht ALLE Kinder erwischen. Die neuen Prozesse scheinen zu starten (kein Fehler in `server_restart.log`), binden aber nicht Port 5000 (SSL-Fehler wird verschluckt). **Symptom: neue Code-Änderungen erscheinen nicht im gerenderten HTML, obwohl eine "neue" uvicorn-Instanz im Background zu laufen scheint.** Diagnose: `powershell -Command "Get-WmiObject Win32_Process -Filter \"Name='python.exe'\""` listet ALLE uvicorn-PIDs (inkl. zombie `multiprocessing-fork`-Kinder), dann `taskkill //PID <pid> //F` für jeden einzelnen. Merkhilfe vor jedem Restart: `powershell ... Measure-Object` muss "Count: 0" zeigen, bevor `nohup uvicorn … &` startet.
